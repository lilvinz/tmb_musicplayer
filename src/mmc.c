...
void cmd_free(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    chprintf(chp,"%lu KB free\r\n",
        (clusters * (uint32_t)MMC_FS.csize * (uint32_t)MMCSD_BLOCK_SIZE)/(1024));
    chprintf(chp,"%lu MB free\r\n",
        (clusters * (uint32_t)MMC_FS.csize * (uint32_t)MMCSD_BLOCK_SIZE)/(1024*1024));
}
